...
--Move to code folder and run alternative code in 10-fasta2phylip.py from the terminal

python 10-fasta2phylip.py

--Run HyDe on chromosome 3 to reproduce Figure 3b

--Create another folder and move all files corresponding to chromosome 3 there 
...
--Run HyDe on each window with the bash script in the code folder
...
--In results, the HyDe output files per position in chromosome 3. We want to extract the triplets, parents, and hybrid belonging to D clade.

--Move to results/HyDe/10Mb-concatenation-ch3
...
--For Figure 3B, we only want the Ae. mutica species from the B clade. This will allow for us to check the hybrid index of D species that come from Ae. mutica.
...
--To recreate the figure in 3A, we need entries with Ae. mutica and T Boeoticum.
...
--Filter our results before plotting
...
--Reproducing something similar to Figure 3A
...
--Do a violin plot of the pvalues
